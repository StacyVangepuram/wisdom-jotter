...
- The "Wisdom Jotter" is an open-source, AI-powered Streamlit application designed to collaboratively collect,preserve and showcase valuable cultural and experiencial knowledge from across India.
- This application serves as a "Corpus Collection Engine" for Viswam.ai's mission to develop AI thta deeply understands India's linguistic and cultural diversity.
- The MVP developed within a one-week sprint allows users to easily contribute either traditional proverbs or life lessons they learnt from their grandparents or parents in their native languages.
- All contributions are stored distinctly, publicly browsable and are checked for semantic similarity using an open-source AI model to minimize redundant data, therby ensuring the collection of high-quality, diverse linguistic and cultural data.
- The applicaton is built with a strong emphasis on accessibility, particularly for users in low-internet bandwidth regions, supporting multilingual input and requiring minimal data transfer.
...
1.4. Technical Architecture & Development
 ...
 - Backend & Data Storage: Python with local JSON files for persistence.
   - Files: data/proverbs.json and data/life_lessons.json are used for storing the collected wisdom.
   - Justification: For the MVP, JSON files offer the simplest setup for persistence without requiring a separate database server, which aligns with the one-week development timeline.

 - Core Libraries:
   - streamlit: For building the interactive web UI.
   - json, os, datetime, uuid: Standard Python libraries for file operations, unique ID generation, and timestamps.
   - pandas: Used for converting the collected JSON data into a tabular format (DataFrame) for easy CSV export.
...
A. Methodology (Planned for Week 2):
 - Recruitment:
   - Leverage personal networks: Friends, family, and colleagues who use smartphones and are comfortable with web Browse.
...
A. Week 1: Rapid Development Sprint (Week: )
 - Plan:
   - Day 1-2: Core Setup & Basic Functionality: Streamlit app scaffolding, data directory/file setup, basic input form for one type (e.g., proverb), saving to JSON, and displaying entries.
   ...
   - Day 5: Feature Expansion (Life Lessons & Separate Storage): Implement radio button for contribution type, conditional input fields, separate JSON storage for life lessons, and distinct display sections.
   ...
   - Day 7: Initial Deployment & Documentation: Deploy MVP to Hugging Face Spaces (ensuring persistent storage is configured), create requirements.txt, README.md, LICENSE, CONTRIBUTING.md, and start drafting REPORT.md.
 ...
 - Key Deliverables (End of Week 1):
   ...
   - Core features: Submit proverbs/life lessons, store in separate JSON files, browse all entries, semantic duplicate detection (using paraphrase-multilingual-MiniLM-L12-v2).
...
C. Weeks 3-4: User Acquisition & Corpus Growth Campaign (Week: )
 ...
 - Target Audience & Channels:
   ...
   - Channels:
     - WhatsApp Groups: Family groups, Friend groups.
       Justification: Active engagement within Indian communities, enabling direct sharing.
     - University/College Student Groups: Especially those in Arts, Humanities, or Linguistics departments.
      Justification: Tech-savvy individuals who can easily access and use the app, and often have projects related to cultural documentation.
...
D. Post-Internship Vision & Sustainability Plan
 ...
 - Scaling Data Collection:
   - Database Migration: Migrate from JSON files to a more robust and scalable database solution (e.g., PostgreSQL, MongoDB) to handle large volumes of data and concurrent users.
